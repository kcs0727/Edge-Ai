...
function RemoveObj() {
  ...
  const { removeobj, buttonLoading } = ApiData();
  const onsubmitHandler = async (e) => {
    e.preventDefault();
    removeobj(input, object, setLoading, setContent);
  }
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start gap-4 flex-wrap text-slate-700'>
      ...
      <form onSubmit={onsubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <p className='mt-8 text-sm font-medium'>Describe single object name to remove </p>
        <input className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder='e.g., watch or spoon, Only single object name' required onChange={(e) => setObject(e.target.value)} value={object} />

        <button type='submit' className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#417DF6] to-[#8E37EB] text-white px-4 py-2 mt-8 text-sm rounded-lg cursor-pointer' disabled={buttonLoading}>
          {
